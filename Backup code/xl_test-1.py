...
import docx
oldVDDpath = "VDD_generator_M145_V5_5_1_Delivery_JIRA Systems and Domains 3-13-2018.xlsx"
newVDDpath = "VDD_generator_M145_V5_5_1_Delivery_JIRA Systems and Domains 3-16-2018 4.xlsx"
...
def write_new_HLS_and_TotalData(HLS, TotalData):
    ...
    wb = xlsxwriter.Workbook("VDD2.xlsx")
...
def open_file():
    ...
    return HLS
#---------------------------------------------------------------
def Update_TotalData_from_HLS(HLS, TotalData):
    TDLines = len(TotalData)
    #TDColumns = len(TotalData[0])
    HLSLines = len(HLS)
    #HLSColumns = len(HLS[0])
    for i in range(0,HLSLines,1):
        if str(HLS[i][3]) != '':
...
def main():
    ...
    #Add function to bring links tab to excel    
    #TotalData = Update_TotalData_from_HLS(HLS, TotalData)
